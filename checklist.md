...
`feat/wgpu-fft-device-validation`. Provider PR #230 merged as `48bb731`; Apollo
STFT then exposed the need to validate a prepared plan's device before its first
queue write. The narrow fix-forward makes the existing ownership check public
and retains the same typed error. Warning-denied all-target Clippy and the
focused real-device FFT suite (6 process tests, all cases green in 15.570
seconds) pass.
...
- [x] Replace initialized host vectors with failure-atomic provider-owned
...
Exact-source MSVC Nextest run `879edae7-437c-4179-8692-f41cc64b212d` passes the
syntax-aware source regression and shared decomposition contract 2/2; the tests
execute in 249 ms and 2.521 s respectively. The initial
Atlas-overlay attempt fails before Hephaestus compilation on a fresh peer-owned
Moirai `missing_docs` ratchet; the committed Git graph compiles cleanly.
Formatting, all-target warning-denied Clippy, doctests 2/2, and warning-clean
...
- [x] Pass focused warning-denied gates and independent review.
- [x] Pass exact implementation-head provider CI.
...
Implementation head `b685cc6` passes CUDA `30731767939`, ROCm `30731767934`,
WGPU `30731767943`, and native macOS Metal `30731767923`; PR #178's docs-only
head must pass the same matrix before merge.
...
- [x] Pass feature-off, warning-denied, focused Nextest, and exact-head CI.
...
- [x] Route complete-pivot LU and column-pivot QR factors through the overwrite
      seam.
...
Implementation owner: Codex on
`codex/hephaestus-decomposition-workspace`. Claimed files are WGPU blocked
Cholesky, LU, and QR implementations; CUDA blocked QR; `CHANGELOG.md`,
`backlog.md`, and this section.
...
ROCm matrix-rank implementations and contracts, plus `CHANGELOG.md`,
`backlog.md`, and this section.
...
Claimed files are the WGPU map-reduction and CUDA/ROCm norm implementations
and contracts, plus `CHANGELOG.md`, `backlog.md`, and this section.
...
- [x] Establish exact blocked Cholesky, LU, and QR value-contract baselines.
- [x] Prove each full-matrix destination is completely copied before first
      decomposition read.
- [x] Route only the nine WGPU, CUDA, and ROCm startup-copy destinations
...
files are the WGPU, CUDA, and ROCm matrix-power implementations and contracts,
plus `CHANGELOG.md`, `backlog.md`, and this section.
...
- [x] Add a standalone Linux WGPU workflow covering minimal and default
      ...
      rustdoc.
- [x] Make CUDA `decomposition` require the CUDA substrate and close the
      invalid feature combination in the audit.
- [x] Synchronize README, changelog, backlog, and checklist evidence.
- [x] Run local gates, then all provider workflows on the exact final head.
...
done. Final head `6d9e96f` passed WGPU run `30187568513` / job
`89754890646` (6m10s), CUDA run `30187568521` / job `89754910876` (7m11s),
ROCm run `30187568515` / job `89754890592` (5m40s), and macOS Metal run
`30187568510` / job `89754890620` (6m57s). NVIDIA hardware job `89754911060`
and AMD hardware job `89754890775` were skipped because hosted hardware
...
- [x] Add deterministic, range, normal-value, and required-device contract
      coverage for the Metal random initializer surface.
- [x] Run the Metal CI feature, lint, nextest, doctest, and rustdoc gates and
...
6m15s. Required-device jobs were skipped because hosted GPU labels were
unavailable.
...
- [x] Add ROCm Hessenberg and real Schur public surfaces behind
      `decomposition`.
- [x] Mirror the CUDA/WGPU Leto provider boundary into typed Q/H and Q/T ROCm
      buffers without a backend-selection fallback.
- [x] Add orthogonality, similarity reconstruction, Hessenberg, real-Schur
      quasi-triangular, spectrum, empty, rectangular, and non-finite value
      contracts.
- [x] Extend hosted ROCm CI and feature hygiene for the final decomposition
      families.
- [x] Synchronize the ROCm README, changelog, ADR, backlog, and checklist.
- [x] Pass hosted build, warning-denied Clippy, Nextest (62/62), doctest, and
      rustdoc at implementation head `6ea7a0c` with run `30135505585`; the
      required-device lane was skipped by the pull-request event.
- [x] Merge the verified increment through PR #86 and re-audit the common
      backend surface.
...
- [x] Add ROCm fluent matrix traits matching the CUDA/WGPU public contract.
- [x] Delegate product, norm, decomposition, solve, property, and matrix
      function methods to the existing ROCm operation families without a
      compatibility adapter or backend fallback.
- [x] Add value-semantic trait-call contracts covering representative dense,
      strided, decomposition, solve, property, and matrix-function cases.
- [x] Synchronize the ROCm README, changelog, ADR, backlog, and checklist.
- [x] Pass hosted build, warning-denied Clippy, Nextest, doctest, and rustdoc
      for the affected feature matrix.
- [x] Publish and merge the verified increment, then re-audit exported common
...
documentation artifacts. Hosted run `30137841431` passed 67/67 tests at
head `356c610`; the local package gate inherits the sibling Leto
...
- [x] Add ROCm `matrix_rank`, `matrix_rank_with_tolerance`, and `det` APIs for
      finite rank-2 strided operands with shared validation.
- [x] Implement one real HIP partial-pivot Gaussian-elimination kernel with
      device scratch storage, rank tolerance, and determinant output.
- [x] Add value-semantic CPU differential contracts for strided full-rank,
      tolerance-boundary, singular, and non-square inputs.
- [x] Synchronize the ROCm ADR, README, changelog, and CI evidence.
- [x] Pass locked feature checks, warning-denied Clippy, Nextest, doctest, and
      rustdoc for the affected package; commit, publish, and merge the
      verified increment.

Execution owner: Codex on the completed ROCm parity branch; completed evidence:
the hosted ROCm container run `30118716147` passed the real feature build,
warning-denied Clippy, Nextest (27/27), doctest, and rustdoc at PR head
`ca602c6`; PR #77 merged as `148436f`. The required-device lane was skipped
for the PR event.

## HEPH-ROCM-PARITY-MATPOW-1 [minor]

- [x] Add the ROCm `matpow` API for square rank-2 strided operands with
      identity handling for exponent zero and typed allocating output.
- [x] Copy non-contiguous inputs through the native strided identity kernel
      and reuse tiled HIP matrix multiplication for exponentiation by squaring.
- [x] Add value-semantic CPU differential contracts for contiguous, strided,
      exponent-zero, and non-square inputs.
- [x] Synchronize the ROCm ADR, README, changelog, and CI evidence.
- [x] Pass locked feature checks, warning-denied Clippy, Nextest, doctest, and
      rustdoc for the affected package; commit, publish, and merge the
      verified increment.
...
Execution owner: Codex on the completed ROCm parity branch; completed evidence:
the hosted ROCm container run `30117981372` passed the real feature build,
warning-denied Clippy, Nextest (25/25), doctest, and rustdoc at PR head
`cd31db6`; PR #76 merged as `809c79f`. The required-device lane was skipped
for the PR event.

## HEPH-ROCM-PARITY-STRIDED-1 [minor]

- [x] Add rank-≤4 ROCm strided binary, unary, and scalar elementwise APIs with
      allocating and caller-owned output forms.
- [x] Implement one packed HIP metadata/decode core with broadcast layout
      mapping and operation-specific binary/unary/scalar kernels.
- [x] Add value-semantic CPU differential contracts for transposed, sliced,
      broadcast, empty, output-alias, and zero-stride output inputs.
- [x] Synchronize the ROCm ADR, README, changelog, and CI evidence.
- [x] Pass locked feature checks, warning-denied Clippy, Nextest, doctest, and
      rustdoc for the affected package; commit, publish, and merge the
      verified increment.
...
Execution owner: Codex on the completed ROCm parity branch; completed evidence:
the hosted ROCm container run `30117107666` passed the real feature build,
warning-denied Clippy, Nextest (23/23), doctest, and rustdoc at PR head
`8699345`; PR #75 merged as `8060f33`. The required-device lane was skipped
for the PR event.

## HEPH-ROCM-PARITY-KRON-1 [minor]

- [x] Add the ROCm strided rank-2 `kron` and caller-owned `kron_into` APIs
      with checked output shape and shared matrix layout metadata.
- [x] Implement one real HIP coordinate-mapping kernel covering arbitrary
      strides and partial output workgroups.
- [x] Add value-semantic CPU differential contracts for strided inputs,
      strided output, zero-sized boundaries, and invalid shape/alias inputs.
- [x] Synchronize the ROCm ADR, README, changelog, and CI evidence.
- [x] Pass locked feature checks, warning-denied Clippy, Nextest, doctest, and
      rustdoc for the affected package; commit, publish, and merge the
      verified increment.
...
Execution owner: Codex on the completed ROCm parity branch; completed evidence:
the hosted ROCm container run `30115666613` passed the real feature build,
warning-denied Clippy, Nextest (21/21), doctest, and rustdoc at PR head
`16cead2`; PR #74 merged as `6ee586f`. The required-device lane was skipped
for the PR event.

## HEPH-ROCM-PARITY-NORMS-1 [minor]

- [x] Add the ROCm strided dot, trace, L1, L2/Frobenius, and max-norm APIs
      with the shared rank-four map-reduction metadata.
- [x] Implement one real HIP map-reduction kernel and compose L2 with the
      existing HIP square-root elementwise kernel.
- [x] Add value-semantic CPU differential contracts for contiguous, offset,
      transposed/strided, diagonal, empty, and invalid-shape inputs.
- [x] Synchronize the ROCm ADR, README, changelog, and CI evidence.
- [x] Pass locked feature checks, warning-denied Clippy, Nextest, doctest, and
      rustdoc for the affected package; commit, publish, and merge the
      verified increment.
...
Execution owner: Codex on the completed ROCm parity branch; completed evidence:
the hosted ROCm container run `30114338471` passed the real feature build,
warning-denied Clippy, Nextest (19/19), doctest, and rustdoc at PR head
`32dc87c`; PR #73 merged as `fb702e9`. The required-device lane was skipped
for the PR event.

## HEPH-ROCM-PARITY-BATCHED-MATMUL-1 [minor]

- [x] Add the ROCm rank-3 batched matrix multiplication API with singleton
      input-batch broadcasting and checked batch-stride metadata.
- [x] Implement the real HIP grid-z tiled batch dispatch with allocating and
      caller-owned output forms, launch chunking, and zero-stride output checks.
- [x] Add value-semantic CPU differential contracts across batches, broadcasts,
...
Execution owner: Codex on the completed ROCm parity branch; completed evidence:
the hosted ROCm container run 30112489093 passed the real feature build,
warning-denied Clippy, Nextest (17/17), doctest, and rustdoc at PR head
5377733; PR #72 merged as 2634776. The required-device lane was skipped for
the PR event.
...
- [x] Add the shared HIP-C dialect and operation/identity expression
      implementations without duplicating the operation vocabulary.
- [x] Implement real ROCm binary, unary, and scalar elementwise kernels using
      hipRTC/module launch and typed `RocmBuffer` ownership.
- [x] Add value-semantic, alias-rejection, and invalid-length contracts that
...
Execution owner: Codex on `codex/hephaestus-rocm-parity`; completed evidence:
the ROCm container run `30105156934` passed the real feature checks,
warning-denied Clippy, Nextest (9/9), doctest, and rustdoc at PR head
`563783f`; PR #67 merged as `7e1fbb9`. The required-device lane was skipped for
the PR event. The claimed files were
...
- [x] Pin separate typed shader bindings and retain custom-identity value
      coverage.
- [x] Place zero/one identity values in separately aligned ranges of one pooled
      uniform allocation.
