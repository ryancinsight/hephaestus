...
- [patch] Allocate rank-2 sum, product, minimum, maximum, and mean outputs
  ...
  omits one initialization transfer of the complete C-contiguous output before
  the axis kernel assigns every logical element. Empty-axis identities/errors,
...
- [patch] Allocate non-empty immediate scalar-reduction outputs and private
  ...
  initialization transfer per native immediate-reduction pass and per private
  prepared-plan intermediate; CUDA
...
- [patch] Allocate non-empty fused map-reduction partials and L2 square-root
  ...
  and one scalar initialization operation per immediate L2 norm. Prepared CUDA
  and ROCm L2 handles retain defined zeroed output because `output()` is
  readable before first dispatch and after a failed dispatch; WGPU and Metal
...
- [patch] Place WGPU matrix-identity zero and one values in separately aligned
  ranges of one pooled uniform allocation, inherited by Metal. Each non-empty
  identity dispatch now acquires two uniform buffers instead of three while
  preserving trait-defined scalar and vector token layouts. Queue writes,
  bindings, output storage, launch count, and arithmetic are unchanged; no
  runtime gain is claimed without matched hardware measurements.
...
- [patch] Allocate non-empty cumulative sum/product and suffix sum/product
  ...
  contiguous result element; WGPU and Metal preserve platform-managed
  initialization behavior. Empty allocations and peak allocation are
  unchanged; no runtime gain is claimed without matched hardware measurements.
...
- [minor] Add shared dialect-specific ZST markers for ReLU, sigmoid, tanh,
  ...
  WGPU, CUDA, ROCm, and Metal export the common vocabulary. Provider CI passed
  the value-semantic f32 contracts on all four backends; Coeus consumer
