...
| `hephaestus-metal` | macOS Metal backend: `MetalDevice`/`MetalBuffer` ownership and the shared application contracts through `hephaestus-wgpu` with the native Metal backend selected, including rank-2 cumulative sum/product scans, seeded uniform/normal initializers, prepared dot/L2, CSR sparse products with prepared SpMV/SpMM batching, direct output-buffer and prepared scalar and rank-2 axis reductions, volume ray integrals, the 2D Laplacian stencil, Metal-owned storage kernels, authored-kernel streams with grouped sequencing, and blocked pivoted decomposition wrappers. |
| `hephaestus-cuda` | CUDA backend: cuda-oxide device acquisition, context binding, `CUdeviceptr` allocation, typed `CudaBuffer<T>`, host/device transfer, and monomorphized elementwise/reduction/scan/linalg/sparse/volume/Laplacian dispatch via ZST op markers, including rank-2 cumulative sum/product scans, reusable prepared dot/L2, scalar, rank-2 axis, and CSR sparse SpMV/SpMM plans with batching, shared geometry and stencil contracts, blocked and pivoted decomposition contracts, and cutile kernel authoring. Dynamic-rank strided elementwise entry points let runtime-shaped consumers delegate their GPU tensor layout kernels without depending on Coeus-local CUDA generators. The `decomposition` feature includes the `cuda` substrate it launches. |
...
- The 2D Laplacian stencil contract is backend-neutral:
  `Laplacian2DParams`, `BoundaryCondition`, and `LaplacianPolarity` live in
  `hephaestus-core`; WGPU, CUDA, and ROCm execute device-resident kernels for
  Dirichlet, Neumann, and periodic boundaries, while Metal delegates through
  its native Metal-selected WGPU device. All providers validate the same grid
  and storage contract.
- The sparse CSR contract is backend-neutral across WGPU, CUDA, ROCm, and
  Metal: each surface owns a typed `GpuCsrMatrix` plus immediate and prepared
  SpMV, SpMM, and multi-RHS SpMV operations. CUDA and ROCm retain native
  compiled kernels and metadata for repeated dispatch; Metal delegates the
  same prepared plans through WGPU's native Metal path. Mixed prepared batches
  preserve submission order without host materialization.
...
- The ROCm decomposition feature runs finite-input validation, Cholesky
  ...
  Bunch–Kaufman permutation. Symmetric Jacobi eigenpairs/eigenvalues and
  general complex eigenvalues use the same provider boundary and typed ROCm
  buffers. The ordinary entry points
  ...
  WGPU, CUDA, and Metal. Complete-pivot LU and column-pivot QR expose the same
  blocked entry-point names on all four backends. Solve, determinant, inverse,
  and least-squares methods retain
...
- All four backend surfaces expose `PreparedReduction` plans for scalar
  sum/min/max. WGPU and Metal retain prepared command resources, while CUDA
  and ROCm preallocate native partial-output trees. Repeated dispatch and
  batch submission stay device-resident; empty identities and block-width
  validation share the same value contract.
- All four backend surfaces also expose `PreparedAxisReduction` plans for
  rank-2 sum/min/max/mean. CUDA and ROCm retain native compiled kernels and
  validated metadata; Metal delegates through WGPU's native Metal path. Both
  immediate and prepared WGPU paths bind a retained identity-only input buffer
  for empty sum axes, while min/max/mean reject undefined empty reductions.
...
cargo test --doc
cargo doc --no-deps
cargo check -p hephaestus-wgpu --no-default-features --all-targets
...
The WGPU, CUDA, ROCm, and Metal lanes include the focused volume and 2D Laplacian
contracts; hardware lanes set `HEPHAESTUS_*_REQUIRE_DEVICE=1` so unavailable
required devices fail rather than silently pass.
...
The WGPU workflow checks both the minimal feature surface and the default
decomposition/sparse surface with warning-denied Clippy, Nextest, doctests,
and rustdoc. CUDA's `decomposition` feature is not an independent host-only
surface: Cargo enables `cuda` with it so every advertised decomposition build
contains the device substrate required by its kernels.
...
sum/min/max and prepared sum/min/max reductions, rank-2 axis sum/min/max/mean reduction,
