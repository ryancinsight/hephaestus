...
| `hephaestus-metal` | macOS Metal backend: `MetalDevice`/`MetalBuffer` ownership and the shared application contracts through `hephaestus-wgpu` with the native Metal backend selected, including rank-2 cumulative sum/product scans, prepared dot/L2, CSR sparse products with prepared SpMV/SpMM batching, prepared scalar and rank-2 axis reductions, volume ray integrals, and the 2D Laplacian stencil. |
