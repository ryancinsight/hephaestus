...
- The ROCm backend owns native HIP device mechanics through the optional
  ...
  vocabulary, while real elementwise, reduction, rank-2 axis-scan,
  ...
  diagonal views. Its default build has no ROCm linkage and returns a typed
...
mechanics, elementwise kernels, reductions, scans, map-reductions, and tiled
matrix-multiplication families belong to `hephaestus-rocm`; sparse, strided
...
rank-2 forward/reverse scan, tiled/batched matrix-multiplication, and strided
