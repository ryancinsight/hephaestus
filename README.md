...
- All four backend surfaces expose immediate rank-2 `prod_axis` and
  `prod_axis_into` operations over the shared `ProdOp` identity contract.
  Allocated and caller-owned forms preserve the reduced axis as length one,
  support strided inputs, and write the multiplicative identity for empty
  reduced axes; Metal delegates through the native Metal-selected WGPU path.
