//! Metal differential contracts for the 2D Laplacian stencil.
...
use hephaestus_metal::{Laplacian2DKernel, MetalDevice};
...
fn device(test: &str) -> Option<MetalDevice> {
    match MetalDevice::try_default() {
        ...
        Err(error) => {
            ...
            eprintln!("skipping Metal stencil contract {test}: {error}");
            None
        }
    }
}
