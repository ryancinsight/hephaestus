    ...
    fn new<const R: usize>(layout: &Layout<R>) -> Result<Self> {
        if R > TENSOR_AXES {
            return Err(invalid(format!(
                "convolution rank {R} exceeds CUDA metadata rank {TENSOR_AXES}"
            )));
        }
        ...
        i32::try_from(minimum).map_err(|_| {
            invalid(format!(
                "layout minimum offset {minimum} exceeds signed CUDA address range"
            ))
        })?;
        i32::try_from(maximum).map_err(|_| {
            invalid(format!(
                "layout maximum offset {maximum} exceeds signed CUDA address range"
            ))
        })?;
...
fn parameters<const S: usize>(values: &[usize; S], name: &str) -> Result<[i32; SPATIAL_AXES]> {
    if S > SPATIAL_AXES {
        return Err(invalid(format!(
            "convolution spatial rank {S} exceeds CUDA metadata rank {SPATIAL_AXES}"
        )));
    }
...
fn validate_product(extents: &[i32], name: &str) -> Result<()> {
    ...
    for (axis, &extent) in extents.iter().enumerate() {
        product = product.checked_mul(extent).ok_or_else(|| {
            invalid(format!(
                "layout {name} element product overflows CUDA i32 at axis {axis}"
            ))
        })?;
    }
    Ok(())
...
fn validate_axis_span(extent: usize, stride: isize, axis: usize) -> Result<()> {
    ...
    if span > i32::MAX as usize {
        return Err(invalid(format!(
            "layout address span {span} on axis {axis} exceeds signed CUDA range"
        )));
    }
    Ok(())
}
