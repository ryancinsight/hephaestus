//! CUDA differential contracts for the 2D Laplacian stencil.
...
use hephaestus_cuda::{CudaDevice, Laplacian2DKernel};
...
fn device(test: &str) -> Option<CudaDevice> {
    match CudaDevice::try_default() {
        ...
        Err(error) => {
            ...
            eprintln!("skipping CUDA stencil contract {test}: {error}");
            None
        }
    }
}
